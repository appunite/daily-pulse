...
from daily_pulse.utils.instances.llm import (
    summarize_individual_long_chain,
    summarize_individual_short_chain,
    summarize_cta_day_chain,
    summarize_no_documents_chain,
)
...
async def getSummarizedAllDayMessage(pages):
    ...
    day_summarize_cta, documents_summarized_long = await asyncio.gather(
        *[getSummarizeCTA(), getSummarizedLong(documents)]
    )
    ...
    documents_summarized_short = await getSummarizedShortOfLong(
        documents_summarized_long_parsed
    )

    documents_summarized_list_stripped = [
        d.replace("\n", "") for d in documents_summarized_short
    ]
    documents_summarized_list = [
        f"`[{i+1}]` {d} `<{pages[i]['url']}|*ADR»*>`"
        for i, d in enumerate(documents_summarized_list_stripped)
    ]
    documents_summarized_combined = "\n\n".join(documents_summarized_list)

    return getSlackMessageAsPlainText(
        day_summarize_title, day_summarize_cta_escaped, documents_summarized_combined
    )
